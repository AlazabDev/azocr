...
للتأكد من الجاهزية قبل أول اختبار تشغيلي، يمكن تشغيل:
```bash
npm run verify   # ESLint + فحص TypeScript بلا إخراج
npm run build    # تحقق بناء الإنتاج
```
...
## الاختبار الأولي
- استخدم دليل `docs/TESTING.md` لخطوات الـ Smoke Test، مع نسخة جاهزة من `.env.local.example` لنسخها سريعاً.
